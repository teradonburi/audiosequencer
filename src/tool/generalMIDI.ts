...
export class GeneralMIDI {
  private _instruments: { name: string; p?: Wave[] }[] = [
    // 1-8 : Piano
  ...
  private _drummaps: { name: string; p?: Wave[] }[] = [
    // 35
  ...
  private _instruments1: Wave[][] = [
    ...
    [
      { w: 'sine', d: 0.7, v: 0.2 },
      { w: 'triangle', v: 4, t: 3, f: 2, d: 0.3, g: 1, k: -1, a: 0.01, s: 0.5 },
    ],
    [
      { w: 'sine', v: 0.35, d: 0.7 },
  ...
  constructor(audioContext: AudioContext) {
    for (let i = 0; i < 128; ++i) {
      this._setTimbre(0, i, this._instruments0[i])
    }
    for (let i = 0; i < this._drumnaps0.length; ++i) {
      this._setTimbre(1, i + 35, this._drumnaps0[i])
    }
    for (let i = 0; i < this._instruments1.length; ++i) {
      this._setTimbre(0, i, this._instruments1[i])
    }
    for (let i = 0; i < this._drummaps.length; ++i) {
      if (this._drumnaps1[i]) {
        this._setTimbre(1, i + 35, this._drumnaps1[i])
      }
    }
    this._createNoise(audioContext)
    this._wave = { w9999: this._createWave(audioContext, 'w9999') }
  }
  ...
  private _setTimbre(m: number, n: number, p: Wave[]) {
    const defp: Wave = {
      w: 'sine',
      g: 0,
      t: 1,
    ...
    function filldef(p: Wave[]) {
      for (n = 0; n < p.length; ++n) {
        for (const k in defp) {
          if (!p[n]?.[k] || typeof p[n][k] === 'undefined') p[n][k] = defp[k]
        }
      }
      return p
    }
    if (m && n >= 35 && n <= 81) this._drummaps[n - 35].p = filldef(p)
    if (m == 0 && n >= 0 && n <= 127) this._instruments[n].p = filldef(p)
  }

  private _noiseBuf: { n0?: AudioBuffer; n1?: AudioBuffer } = {}
  private _createNoise(audioContext: AudioContext) {
    const sampleRate = audioContext.sampleRate
    const blen = (sampleRate * 0.5) | 0
    const convBuf = audioContext.createBuffer(2, blen, sampleRate)
    const d1 = convBuf.getChannelData(0)
    const d2 = convBuf.getChannelData(1)
    this._noiseBuf.n0 = audioContext.createBuffer(1, blen, sampleRate)
    this._noiseBuf.n1 = audioContext.createBuffer(1, blen, sampleRate)
    const dn = this._noiseBuf.n0.getChannelData(0)
    const dr = this._noiseBuf.n1.getChannelData(0)
  ...
  private _wave: PeriodicWave = null
  private _createWave(audioContext: AudioContext, w: string) {
    const imag = new Float32Array(w.length)
    const real = new Float32Array(w.length)
    for (let i = 1; i < w.length; ++i) {
      imag[i] = parseInt(w[i], 10)
    }
